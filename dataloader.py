...
class BinaryLoader(Dataset):
        ...
        def __getitem__(self,idx):
            ...
            density_path = os.path.join(self.path, 'DensityMap', image_id)
    
            img = io.imread(image_path+'.png')[:,:,:3].astype('float32')
            mask = io.imread(mask_path+'.png', as_gray=True)
            density_map = io.imread(density_path + '.png', as_gray=True).astype('float32')
            
            data_group = self.transforms(image=img, mask=mask, density_map=density_map)
            img_resized = data_group['image']
            mask = data_group['mask']
            density_map = data_group['density_map']

            #convert img to tensor and use preprocess to normalise img
            img = ToTensor()(image=img)['image']
            img = self.preprocess(img)
   
            return (img_resized, img, mask, image_id, density_map)
